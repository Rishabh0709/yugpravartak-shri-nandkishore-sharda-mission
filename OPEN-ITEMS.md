...
- ~~Student headcounts~~ — **done** (2026-09-06). The girls figures went through too many revisions (37,000 → 8,000 → 8,400 → 9,031 → …) to stand behind publicly, so student counts are gone from every page **except impact.html**, where two survive by your call: the combined **8,000+ girls** figure (SVT + Maa Shardamani, in the Maa Shardamani detail-record highlight) and **631 boys** (Gyanyogi detail record — KPI + summary). Scrubbed from: impact.html topline (now a 2-stat strip), SVT + Maa Shardamani trust pages, Gyanyogi trust page, index-v2 + index. Both live figures are hard-coded in `impact-data.json`; `impact.json`'s student metrics are internal reference only now (see its `_note`).
