...
- ~~Student headcounts~~ — **removed from display** (2026-09-06). The girls figures went through too many revisions (37,000 → 8,000 → 8,400 → 9,031 → 8,000 → …) to stand behind publicly. Pages now lean on the ₹ scholarship amounts + the 45-schools / 32-colleges reach. Removed from: impact.html topline (now a 2-stat strip), the Maa Shardamani highlight line, SVT + Maa Shardamani trust pages (hero + stat bands), index-v2 + index seva-card metas / stories summary. **studentsBoys (631) is still shown on the Gyanyogi page** — you named only SVT + Maa Shardamani; say if you want that gone too for consistency. `studentsGirls`/`studentsCombined` kept in `impact.json` as internal-only reference (see its `_note`).
